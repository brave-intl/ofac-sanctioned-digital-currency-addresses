...
def generate_actions(true_list, mirror_list):
    ...
    for needs_add in only_in_true:
        action_list.append({"action": "add", "address": needs_add})
    for needs_remove in only_in_mirror:
        action_list.append({"action": "remove", "address": needs_remove})
    return action_list
...
def process_action_chunk(action_chunk, bucket, prefix, dry_run, s3_client):
    ...
    results = {"created": 0, "removed": 0, "errors": 0}
...
def reconcile_s3(
    ...
    logger.info(
        f"Starting to take {total_actions} actions on S3 objects using "
        f"{workers} worker threads"
    )

    # Create chunks of actions to process
    action_chunks = [
        action_list[i : i + chunk_size] for i in range(0, len(action_list), chunk_size)
    ]
    ...
    if total_count > 0:
        ...
        if percent_removed > 15:
            ...
            if os.getenv("GITHUB_ACTOR") not in ["mrose17", "Sneagan", "mschfh"]:
                logger.error(
                    "Too many addresses are set to be removed. Human "
